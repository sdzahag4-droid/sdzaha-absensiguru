...
window.onload = function() {
    // 1. Ambil data identitas guru yang sedang login (misal dari localStorage)
    const nipGuru = localStorage.getItem('nip') || sessionStorage.getItem('nip');
    
    // 2. Format tanggal hari ini (YYYY-MM-DD)
    const today = new Date().toISOString().split('T')[0];

    if (nipGuru) {
        // 3. Lakukan pengecekan ke database / spreadsheet apakah sudah absen hari ini
        fetch(`https://script.google.com/macros/s/AKfycbz4kpL6xEXXBkBO2aOGJGCKwFAYdk6Jwt0_8_WskgH8HvuECRg6MVIJlaxgDMAeIk_U/exec`)
            .then(response => response.json())
            .then(data => {
                if (data && data.sudahAbsen) {
                    // Jika sudah absen, nonaktifkan tombol-tombol pilihan absen
                    const tombolAbsen = document.querySelectorAll('.btn-absen'); 
                    tombolAbsen.forEach(btn => {
                        btn.disabled = true;
                        btn.style.opacity = '0.5';
                    });
                    
                    console.log("Guru sudah melakukan absensi hari ini.");
                }
            })
            .catch(err => console.error("Gagal mengecek status absensi:", err));
    }
};